...
``${DBMASK_SEED_SALT}``); it is then never written to disk and the store alone
...
DEFAULT_SEED_MAP_URL = "sqlite:///dbmask_seedmap.db"
...
seed_map_table = Table(
    ...
    UniqueConstraint("scope", "value_hash", name="uq_dbmask_seed_pair"),
)
...
class SeedStore:
    ...
    Usage::
        ...
        store = SeedStore(url="sqlite:///seedmap.db", secret="dbmask")
        store.connect()
